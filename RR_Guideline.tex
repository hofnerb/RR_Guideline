...
% \title{Biometrical Journal -- Guideline for code submissions\\[-0.5em]}
...
\item Each user-defined function {must} be documented (including all
  function arguments).
\item Unnecessary comments (such as alternative code lines) {should} be
  avoided.
\item Please indent for your code properly. 
  \begin{itemize}
  \item If you plan to provide \textsf{R} code and use RStudio
  ...
  \end{itemize}
\item Try to put the code in as few code files as possible\footnote{But use
    separate files where useful and sensible, e.g., one for the function
    definitions, one for the simulation study and another for the data
    analysis.}.
  \begin{itemize}
  \item Do not use seperate files for each function, but do structure the code
    files properly using comments for each "section" (unless you use, e.g.,
    \textsf{MATLAB} where one file per function is required).
  \end{itemize}
\item Read other files, such as the function definitions, into the main file
  (e.g., using \texttt{source()} in \textsf{R}).
...
\item Avoid absolute paths (e.g., \verb+C:/My Code+) whenever possible. Use
  ...
  \begin{itemize}
  \item   In \textsf{R} absolute paths are not necessary (unless in come very rare
    occasions). If you really want to set or change your working directory do this
    only once, at the very beginning of your master code file.
...
\end{itemize}
\item If the code needs more time to run (say longer than 2h), consider to add
  ...
  the folder ...'').
\item If the data is confidential, authors should consider to use simulated data
...
\item Each code submission {must} be accompanied by a \texttt{README.txt} or a
  ...
  \item a list of configurations on which you tested your code [software,
    software version (incl. package versions), platform]. In \textsf{R} you can
    use the output from \texttt{sessionInfo()} at the end of your code, i.e.,
...
\item \textbf{After (conditional) acceptance:}\\[0.5em]
  ...
  manuscript was changed please send also the manuscript (incl. sources) per
...
are encouraged. Please also supply an \textsf{R} code file in that case.
